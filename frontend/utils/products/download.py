...
async def download(s2_uri):
    gcs_s3 = S3FileSystem(anon=True, endpoint_url=S3_GCS_ENDPOINT)

    vkr = LocalFileSystem()

    copier = AsyncS3Copy(gcs_s3, vkr)
    ...
    await copier.copy_directory(
        s3_esa_s2_location, f"/opt/satellite-data/{s2_uri}"
    )
